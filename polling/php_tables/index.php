<?
	...
	$json_string = '
		...
		[
			...
			["THERM2A","THERMS: BETH MCGINN",[["1||2","TOTAL FAV"],["3||4", "TOTAL UNFAV"], ["(1||2) -(3||4)", "TOTAL FAV - TOTAL UNFAV"],[1,"VERY FAVORABLE"],[2,"SOMEWHAT FAVORABLE"],[3,"SOMEWHAT UNFAVORABLE"],[4,"VERY UNFAVORABLE"],[5,"NO OPINION/NEUTRAL"],[6,"NEVER HEARD"],[7,"DK/NR"]]],
			["THERM2B","THERMS: RUTH BRIGGS KING",[["1||2","TOTAL FAV"],["3||4", "TOTAL UNFAV"], ["(1||2) -(3||4)", "TOTAL FAV - TOTAL UNFAV"],[1,"VERY FAVORABLE"],[2,"SOMEWHAT FAVORABLE"],[3,"SOMEWHAT UNFAVORABLE"],[4,"VERY UNFAVORABLE"],[5,"NO OPINION/NEUTRAL"],[6,"NEVER HEARD"],[7,"DK/NR"]]],
			["THERM2C","THERMS: JACK MARKELL",[["1||2","TOTAL FAV"],["3||4", "TOTAL UNFAV"], ["(1||2) - (3||4)", "TOTAL FAV - TOTAL UNFAV"],[1,"VERY FAVORABLE"],[2,"SOMEWHAT FAVORABLE"],[3,"SOMEWHAT UNFAVORABLE"],[4,"VERY UNFAVORABLE"],[5,"NO OPINION/NEUTRAL"],[6,"NEVER HEARD"],[7,"DK/NR"]]],
			["THERM2D","THERMS: JACK JEFFREY CRAGG",[["1||2","TOTAL FAV"],["3||4", "TOTAL UNFAV"], ["(1||2) -(3||4)", "TOTAL FAV - TOTAL UNFAV"],[1,"VERY FAVORABLE"],[2,"SOMEWHAT FAVORABLE"],[3,"SOMEWHAT UNFAVORABLE"],[4,"VERY UNFAVORABLE"],[5,"NO OPINION/NEUTRAL"],[6,"NEVER HEARD"],[7,"DK/NR"]]],
			["JOBA","JOB APPROVAL 13 GOVERNOR JACK MARKELL",[["1||2", "EXCELLENT/GOOD"], [1,"EXCELLENT"],[2,"GOOD"],[3,"JUST FAIR"], [4,"POOR"], [5,"DK/NR"]]],
			["JOBB","JOB APPROVAL 14 STATE REP RUTH BRIGGS KING",[["1||2", "EXCELLENT/GOOD"], [1,"EXCELLENT"],[2,"GOOD"],[3,"JUST FAIR"], [4,"POOR"], [5,"DK/NR"]]],
			...
			[["GOVVOTE, GOVLEAN"],"GOVERNOR RACE", [["GOVVOTE ==1","JACK MARKELL"], ["GOVLEAN == 1", "LEAN JACK MARKELL"], ["GOVVOTE == 2", "JEFFREY GRAGG"], ["GOVLEAN == 2", "LEAN JEFFREY CRAGG"], ["GOVVOTE == 3 && GOVLEAN == 3", "UNDECIDED"], ["GOVVOTE == 1 || GOVLEAN == 1", "TOTAL JACK MARKELL"], ["GOVVOTE == 2 || GOVLEAN == 2", "TOTAL JEFFREY CRAGG"], ["(GOVVOTE == 1 || GOVLEAN == 1) - (GOVVOTE == 2 || GOVLEAN == 2)", "TOTAL MARKELL - CRAGG"]]],
			[["VOTE1,VOTE1B"], "INITIAL VOTE", [["VOTE1 == 1","BETH MCGINN"], ["VOTE1B == 1", "LEAN BETH MCGINN"], ["VOTE1 == 2", "RUTH BRIGGS KING"], ["VOTE1B == 2", "LEAN RUTH BRIGGS KING"], ["VOTE1 == 3 && VOTE1B == 3", "UNDECIDED"], ["VOTE1 == 1 || VOTE1B == 1", "TOTAL BETH MCGINN"], ["VOTE1 == 2 || VOTE1B == 3", "TOTAL RUTH BRIGGS KING"], ["(VOTE1 ==1 || VOTE1B == 1) - (VOTE1 == 2 || VOTE1B == 2)", "TOTAL MCGINN - KING"]]],
			[["VOTEBIO,VOTEBIO2"], "INFORMED BIO VOTE", [["VOTEBIO == 1","BETH MCGINN"], ["VOTEBIO2 == 1", "LEAN BETH MCGINN"], ["VOTEBIO == 2", "RUTH BRIGGS KING"], ["VOTEBIO2 == 2", "LEAN RUTH BRIGGS KING"], ["VOTEBIO == 3 && VOTEBIO2 == 3", "UNDECIDED"], ["VOTEBIO == 1 || VOTEBIO2 == 1", "TOTAL BETH MCGINN"], ["VOTEBIO == 2 || VOTEBIO2 == 3", "TOTAL RUTH BRIGGS KING"], ["(VOTEBIO ==1 || VOTEBIO2 == 1) - (VOTEBIO == 2 || VOTEBIO2 == 2)", "TOTAL MCGINN - KING"]]],
	...
	foreach ($x as $thisGroup) {
		...
		$totalSql = "SELECT count(*) FROM data";
		...
		foreach($thisGroup[2] as $thisQuestion) {
			...
			if (!is_array($thisColumn)) {
				...
				if (stristr($thisValue, "||") == FALSE && stristr($thisValue, "&&") == FALSE) {
					...
					$sql = "SELECT count(*) FROM data WHERE ".$thisColumn." = ".$thisValue;
					...
					// (1||2) - (3||4) =>  select( (SELECT COUNT(*) FROM data where reelect = 1 or reelect = 2) - (SELECT COUNT(*) FROM data where reelect = 3 or reelect = 4) )
					
					$sql = "SELECT (";
					
					$patterns = array('/([A-Za-z0-9=]*)(\|\|)([A-Za-z0-9=]*)/i', '/([A-Za-z0-9=]*)(&&)([A-Za-z0-9=]*)/i', );
					$replacements = array(' select count(*) from data where '.$thisColumn.' = $1 OR '.$thisColumn.' = $3',
											' select count(*) from data where '.$thisColumn.' = $1 AND '.$thisColumn.' = $3');
				...
				if(stristr($thisValue, ") - (") == FALSE && stristr($thisValue, ")-(") == FALSE){
					...
					$sql = "select count(*) from data where ".$thisValue;
					...
					$replacements = array(' (select count(*) from data where ($1)) - (select count(*) from data where ($2)) ');
					$thisValueOut = preg_replace($patterns, $replacements, $thisValue);
